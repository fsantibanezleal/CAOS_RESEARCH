...
\fancyhead[R]{\small Planar Keller Maps \textperiodcentered\ v0.22}
...
Paper B of a three-paper record \textperiodcentered\ version 0.22 \textperiodcentered\ 2026-07-29\\
...
ideals close \(G\). The \(L\) and \(Q\) common residuals remain finite. These
are strict chart results, not closure of the full \(T_B\) restriction or the
full family.
...
The corresponding ideals on \(L\) and \(Q\) are nonunit but
...
Together, EXP-118--120 close the \(d=0\) quotient boundary and the \(G\)
open component, but not the \(L/Q\) finite residuals. They do not close the
full \(T_B\) restriction, the 24-parameter core, the full 51-parameter
...
minor row bases directly on the finite \(L\) and \(Q\) common residuals left
by EXP-120, and test the resulting ideals exactly; (2) retain EXP-118's
complete \(d=0\) cover and EXP-120's \(G\)-component cover as regression
gates rather than recomputing them; (3) retain the exact EXP-108 lift through
