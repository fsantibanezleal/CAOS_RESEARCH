...
\fancyhead[R]{\small Planar Keller Maps \textperiodcentered\ v0.19}
...
Paper B of a three-paper record \textperiodcentered\ version 0.19 \textperiodcentered\ 2026-07-26\\
...
Priorities, from the standing routes evaluation: (1) lift the exact EXP-108
cover through the next direction \((0,6)\), using
\(x=\varepsilon_{(0,6)}/u^2\) so its matrix term is \(zxA_{(0,6)}\), and
recurse only on residual closed strata; (2) reopen the intersection-$21$ transport only
