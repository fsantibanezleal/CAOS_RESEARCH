...
\fancyhead[R]{\small Planar Keller Maps \textperiodcentered\ v0.17}
...
Paper B of a three-paper record \textperiodcentered\ version 0.17 \textperiodcentered\ 2026-07-26\\
...
nonzero (EXP-102). Complete coverage of this curve remains open: the dense
third-minor pullback has combined modular direction rank \(121\) and hit its
declared five-minute budget. The next exact object is the determinantal
divisor of the cleared polynomial matrix over \(\mathbb{Q}[u]\), not another
dense bivariate expansion.

These results establish the certificate mechanism and the first exact chart
transition on a two-parameter slice. They do not cover the other parameters,
exclude \((72,108)\), or raise the degree floor.
...
Priorities, from the standing routes evaluation: (1) compute the
determinantal divisor on the EXP-101 residual curve over
\(\mathbb{Q}[u,u^{-1}]\), then enlarge the constructible stratification
coefficient block by coefficient block; (2) reopen the intersection-$21$ transport only
after constructing a complete boundary-divisor ledger through the swap,
Laurent cuts, and final inversion; (3) pursue Newton resolution only after deriving a
condition beyond the already-retained first $D=72$ branch; and (4) retain the
