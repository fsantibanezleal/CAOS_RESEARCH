...
% Computation identifiers (EXP-nnn) are kept whole at line ends with \mbox; the emergency
% stretch lets the few paragraphs that then have no good break points set without overfull lines.
...
\newcommand{\docversion}{v0.34}
\newcommand{\docdate}{18 September 2026}
\newcommand{\versiondoi}{10.5281/zenodo.22834215}
\newcommand{\conceptdoi}{10.5281/zenodo.21503367}
...
\fancyhead[R]{\small Planar Keller Maps \textperiodcentered\ \docversion}
...
$JC(2)$ as the open case. This paper develops an exact-arithmetic program on the planar
...
\emph{unconditionally} at all partner degrees (cleared base certificates with monomial pairings
such as $-13860\,a^7$, plus a resonance-reduction lemma showing window growth can never
repair the constant's row). A recalibration against the primary sources shows that our
composite-gcd certificates replicate literature-covered territory (the verified
floor is $\gcd \le 8$, primes, $2p$, then Heitmann's $B \ge 16$ and the
Guccione--Guccione--Valqui dichotomy $B = 16$ or $B > 20$), and that the open frontier is the
$B = 16$ normal form together with the single surviving degree pair $(72, 108)$ below $125$.
For that open pair, an exact instantiation of the primary sources gives two necessary
conditions on the original pair: seven possible northeastern vertices for a nonzero inner
polynomial, and $\deg_x\operatorname{Res}_y(P,Q)=21$ with an $84+24$ major/minor root
partition; neither condition alone excludes the pair.
A constructible determinantal-strata calculation closes the first actual
three-coefficient GGHV slice exactly. On the residual curve, two normalized
maximal minors reduce, with \(z=u^9\), to
\(21-96z-1024z^2\) and \((8z+1)^{14}\); an explicit integer Bezout identity
...
nonconstant direction in the 26-parameter lower family. This is a strict slice
result and does not exclude the full \((72,108)\) family.
At the level of the full family, after removal of the structural constant column
and with the complete 302-row pool (the forced-only construction has 289 rows),
one selected augmented minor compresses to a 36-column block depending on 24 of the
51 parameters. Two
...
A distinct exact section on \(F_7\) has a degree-58 norm. Its \(B^{16}\)
factor lies on \(A=0\), and its degree-12 factor lies on \(R=S=0\);
the surviving irreducible degrees \(3,9,18\) give 30 normalized values and
90 lifts. Exact CRT comparison then identifies the degree-9 and degree-18
blocks with the same geometric points already projected from \(F_3\) and
\(F_6\), reducing the seven factor appearances to five squarefree blocks of
total degree 75. The \(h_7\) section covers the unique degree-15 and degree-30
blocks. Two further exact maximal-minor sections generate the unit ideal on
the remaining \(F_7\) blocks of degrees \(3,9,18\). Hence the accumulated
atlas closes the entire \(AS\ne0\) rational graph. Exact factorwise
subresultants then identify the principal-open base locus as a reduced
90-dimensional algebra with field-block degrees \(3,6,12,69\). Existing
sections cover the degree-12 and degree-69 blocks; one new exact section
completes degrees 3 and 6. Bezout identities, independent resultants,
multiplication-matrix norms, and a direct determinant control certify the
finite atlas uniformly in \(Y\). Thus the complete \(A\ne0\) sector of the
four-parameter restriction is covered. Returning to the original
302-by-125 augmented matrix before invariant division, the direct \(A=0\)
boundary admits two exact minors independent of \(C\). Their divisors are
\((5B+4)^3(25B^2-20B+16)^3\) and
\(B^{95}(109375B^6-110592)(21875B^6-4800B^3-24576)\), up to nonzero
scalars. An explicit Bezout identity makes their gcd one. Combined with the
\(d=0\) quotient cover, this closes the complete four-coefficient restriction.
Adjoining the transverse coefficient at \((2,8)\), EXP-132 closes the entire
direct \(A=0,d=1\) boundary of that five-coefficient restriction. Three
normalized maximal minors generate the unit ideal in \(\Q[B,C,T]\): after
one exact row combination, two depend only on \(B\) and are coprime, with an
explicit characteristic-zero Bezout certificate [MV/D]. The inherited
EXP-123 and EXP-124 bases vanish identically on this specialization, so the
certificate requires a new SCC-guided basis rather than a parameter
substitution into the four-coefficient atlas. EXP-135 then proves that the
EXP-124 section is unchanged by \((2,8)\) on its complete normalized
\((A,B,C)\) ambient chart. The transverse core has ranks \(7,3,0\) through
its first three powers. Separate determinant degree bounds
\((25,24,6,7)\), a complete 136,500-control tensor grid over 30 primes, and
a CRT modulus exceeding the explicit characteristic-zero coefficient bound
certify \(\det(H+T K_{(2,8)})=\det(H)\) [MV/D]. Hence the
\(F_3F_6F_7\) divisor of the four-coefficient restriction is retained for this
section. The other residual
sections and the transverse \(d=0\) quotient remain open, so the complete
five-coefficient restriction and every larger target remain open.
...
(Newton polygons, approximate roots, gcd coverage) is its context. The three-dimensional
counterexample itself (validation, the seed family, the geometry of non-properness,
two-dimensional equivariant rigidity) is studied in the companion paper
\cite{SLfoundational}; this paper treats the planar case. Every computer-assisted statement
cites the archived computation that establishes it (EXP-$nnn$; see the section on data and
code availability).

Labels: \textbf{[MV]} machine-verified exact identity; \textbf{[D]} derived analytically and
certified on instances; \textbf{[C]} conjecture. Machine verification is exact arithmetic:
sympy over $\Q$ or $\Q(i)$, and modular arithmetic over prime fields with Chinese-remainder
reconstruction and explicit degree and height bounds where a statement over $\Q$ is derived
from modular computations; no floating-point value enters a certificate.
...
pairings for out-of-window preimages) reproduces the computed values exactly.
\end{lemma}
...
emit its cokernel obstructions) is therefore well-defined; it reproduces the outcome of the
monolithic consistency test on every tested sample, including a consistent positive control. On the
...
and any $a \ne 0$, $b$: $P = x + a\,x^u y^v + b\,x^d$ is not a Keller component at \emph{any}
...
$T = P_D$ is \emph{not} a proper power. If the completion window is inconsistent with a
certificate at any single $N \ge \deg P - 1$, then $P$ is not a Keller component at \emph{any}
...
of computation, yields an all-degree exclusion per shape; the shapes excluded in this way
include multi-edge staircases such as $x + a x^2 y + b x^2 + c x^3 y^2$ (pairing
$-4620\,c^4$). For proper-power tops the measured mechanism is the \emph{half-plane}
certificate \mbox{(EXP-048)}: a pairing-nonzero certificate supported in the $y$-heavy half-plane,
disjoint from every $x$-heavy resonance source; its general construction is the remaining
structural gap, and it is exactly what the $B = 16$ frontier shapes (proper-power tops)
require. Certificates also exist at the open pair's degrees themselves:
...
subsystem. Hence \emph{one} $H$-certificate at one window excludes $P$ at \emph{all} partner degrees,
proper-power tops included. At the frontier, the degree-32 $B = 16$-flavor sample and
...
The statements about the literature in this section were checked against the cited
sources as of 2026-07-22.
...
used. Consequently our machine certificates at $\gcd = 2, 9, 12, 18$ (EXP-024..028) are
\emph{replications} of literature-covered territory: independent verifications, not new
frontier results.
...
which the algorithmic re-examination by Guccione--Guccione--Horruitiner--Valqui enumerates
and discards \cite{GGHV2017, GGHV2022}. The current floor is: any counterexample has
...
\emph{The open targets.} (i) The $B = 16$ case comes with a forced direction set and
...
$(48, 64)$ full-window system from $2142$ to $111$ unknowns: three degree-48 samples of the
$F_1$ flavor (the pure shape $x + R_0(1)^3$ and two swallowed variants) have \emph{empty}
filtered windows at $N = 64$ in $2.4$ to $3.6$ seconds each, excluding all partners of
degree $\le 64$ per sample (the classical case-64 degrees: a sampled replication of Moh and
Heitmann that also measures the computational cost at this scale). (iii) For the $(72, 108)$ system the histograms give $5992$ unknowns
...
The lone surviving degree pair below $125$ reduces (Guccione--Guccione--Horruitiner--Valqui
\cite{GGHV2022}, Prop.~4.3; transcription verified against the LaTeX sources)
to the existence of a pair with $[P, Q] = x^2$ supported in \emph{small} polygons:
\[
\begin{aligned}
...
\end{aligned}
\]
The original paper's unsolved system was never printed. Computations EXP-050 through
\mbox{EXP-055} give the following. Sampled shapes at the original degrees and on the reduced
polygons are all partner-free (seconds per certificate). On the stratum with the forced top
edge $y^8 (xy - t)^8$ symbolic, every cleared certificate pairs to the \emph{constant} $576$
(all $t$, identical across sampled lower strata; five-row support), but that covector is not
rigidly universal: 51 lower combinations enter (EXP-054). This leads to the perturbative
construction $\Lambda(\varepsilon)$; its base is sound (the right-hand side is the constant
$x^2$ vector; $\Lambda_0$ globally left-null, symbolic $t$) and \emph{first-order
universality} is proved: all 26 entering lower monomials admit correctors with the $576$
pairing pinned and localized supports. Order-1 termination fails (measured), so the corrector
ladder continues at order 2; each rung is a batched linear solve, and termination at any
finite order yields the explicit universal covector, closing the reduced stratum for all
parameters. Closing the reduction's other forcing branches as well would then raise the
floor from $108$ to $125$.

\subsection*{The three reduced branches of the $(72, 108)$ case}

All three branches of the GGHV reduction are certificate-covered on their forced families. Cases a) and b) (the sub-polygon systems) close with monomial
...
with identical outcome): the $\beta$-symbolic certificates give pairing
...
The orientation swap is absorbed by $Q \to -Q$. Since the GGHV reduction forces \emph{no}
interior coefficients, a closure must hold for \emph{all} interior values, and
axis-symbolic coverage together with sampling does not establish that: extrapolation from
slices fails in this system (the covector with constant pairing $576$ on sampled strata is
not universal, EXP-054). The certified statement is therefore: \textbf{the three reduced
branches are certificate-covered on their forced-edge families, with interior
generality axis-symbolic and densely sampled: strong evidence toward, but not a proof
of, the $(72, 108)$ closure}; raising the floor to $125$ requires a certificate that is
simultaneously symbolic in all interior coefficients.

\subsection*{The truncation ladder for the simultaneous certificate}

The certified statement above reduces raising the floor to one object: a covector
$\Lambda(\varepsilon)$, polynomial in the interior coefficients $\varepsilon$,
that closes the reduced system for \emph{all} interior values at once. The
computations below decide this object degree by degree; every step is
...
constructive solve remain open, subject to the one-sidedness noted below. The
...
(single operators at degree one; at degree two every pair support is feasible,
EXP-071, and a triple obstructs).

\emph{One structural reduction, and no degree bound.} The rational-covector
...
polynomial one. The dimension of the Krylov closure of $\Lambda_0$ is \emph{not}
known to bound the degree of a covector (EXP-078): the derivation of such a finite
ceiling presupposes that the pinned corrector ladder terminates, and the ladder does
\emph{not} terminate (EXP-064: the descending chain stabilizes nonzero at dimension
$39$). Since the pinning carries a large gauge freedom, neither
the ceiling nor its negation follows; \emph{no} a priori degree bound is known,
effective or otherwise, the chain of solvable degrees is controlled only by
Noetherianity, which is not effective, and the degree-by-degree search is not known
to be a finite decision procedure.

What does hold is weaker but exact. The set of degrees at which a covector exists
...
requires constructing a covector, not sweeping. Both outcomes remain open: a
covector would be an exclusion proof, and a proof that none exists at any degree
would be the first step of a counterexample.
...
ladder). Whether this decides $(72,108)$ in one stroke is open; we state it as a
question, not a result.

\subsection*{Necessary conditions from the original pair (EXP-095/096)}
...
These are necessary conditions on the original pair, independent of sampling the
$51$ interior coefficients of the reduced system. They do not prove that any of
...
\emph{Transport to the reduced system (EXP-097).} These original-pair invariants
...
necessary condition for reconstructing an original polynomial pair, not a direct
equation on the reduced coefficients. Laurent exponent width is a conditional
replacement only after proving $\nu=0$ and swap compatibility. A complete account
of the boundary divisors through these steps could supply that missing theorem;
none is claimed here.

\subsection*{Constructible certificate strata and the first exact chart transition}

Finite chart covers by localized certificates need a commutative-algebra
qualification \mbox{(EXP-098)}. Suppose principal opens \(D(s_i)\)
...
This mechanism applies to the actual GGHV matrix (EXP-099--101). Common-flag
shortcuts fail, but they isolate the first nontrivial interior interaction:
\[
...
\]
At \(u=1\), both previous minors vanish but a third exact augmented minor is
nonzero \mbox{(EXP-102)}. Its dense pullback has combined direction rank \(121\) and
exceeded a five-minute time limit. \mbox{EXP-103} instead evaluates at roots of unity
and interpolates the complete maximal-minor polynomials by NTT. An endpoint
check detects an 81-degree cancellation that a modular gcd alone would miss.

EXP-104 settles this cancellation over characteristic zero [MV]. The third determinant
...
9 \mbox{(EXP-105)}. With \(z=u^9\), removal of the proved Laurent monomials and
...
EXP-105 exhibits integer polynomials \(A,B\) satisfying
\[
...
rank \(125\) for the augmented matrix at every point of the
\(\{(0,1),(1,7)\}\) coefficient slice [MV].

The grading is not confined to that slice. For every nonconstant lower
monomial \(x^py^q\) tested in the 26-parameter family, EXP-106
...
gradings; the constant direction is bracket-zero. This supplies a coordinate
system that scales to further directions, not a full-family proof. The next
direction, \((0,7)\), has selected ranks 53 and 41; after setting
...
\end{split}
\]
Their exact gcd is one. The explicit integer B\'ezout identity has nonzero
constant
\[
...
\subsection*{Full-family rank and the exact 36-column core}

For the full family, full rank of the coefficient matrix is an unattainable target.
The \(Q\)-polygon contains the constant monomial, whose column is identically zero:
\[
[P,1]=0.
\]
...
common zero locus of augmented maximal minors. For the row pool (EXP-111), the
forced-only construction has 289 rows, while the complete union inside the same
reduced pool has 302 rows [MV].
...
They expose a weighted open/boundary split for the chart calculations below:
...
51,11,10,9,8,7
\]
and sixteen cyclic singletons. EXP-117 computes the determinants of the
51-block and of all smaller blocks. In original coordinates the selected
quotient determinant is, up to an explicit nonzero rational scalar,
\[
\det A_{\mathrm{quot}}
\doteq
b^{32}F_{28}(a,b)
...
\]
The scalar is a nonzero integer. This chart covers \(a\ne0\),
...
\]
Factorwise exact Gr\"obner calculations give
\[
...
zero-dimensionality; an FGLM conversion exceeded a 180-second time limit, so
no lex eliminant or point count is claimed. The explicit
...
\]
For \(Q\), a direct Gr\"obner calculation on the three generators exceeded a
240-second time limit, so the certificate uses the exact factor
...
restored direction contributes at first order or pairwise mixed order. Hence, on
this basis, no restored direction lifts for free.
...
characteristic-zero determinant, the check that its specialization at \(C=0\)
reproduces the complete EXP-121 determinant, and four direct full-matrix rational
controls.
...
\[
 Y=-\frac{R(X,B)}{S(X,B)}.
\]
The base locus \(V(R,S)\subset\mathbb A^2_{X,B}\) is
...
By itself it is not a four-parameter chart cover: alternative minors must
cover the rational graph, the finite base locus, and the separate divisor
\(A=0\). The graph and the base locus are closed below.
...
\[
 F_3(X,B)=(5B+4)^3+16X.
\]
Suitable primes are not automatic: cubic-residue and factor-point obstructions
exclude some primes, and sample points must lie on the cube locus because
\(X=A^3\) is required. The primes 739 and 811 satisfy these conditions. On both primes,
...
[MV]. The EXP-125 \(F_6\) basis is exactly the \(F_3\)-selected basis
...
EXP-127 applies the same dictionary to the distinct EXP-125 \(F_7\) basis
[MV]. Its exact anchor has a 31-vertex largest cyclic block. The complete
determinant has \(A\)-valuation 89 and, after invariant reduction, is
independent of \(Y=A^2C\). Modulo the irreducible quadratic-in-\(X\) curve
\(F_7\), its primitive class is again linear:
\[
 h_7(X,B)=U_7(B)X+V_7(B).
\]
The Sylvester resultant and quotient-multiplication determinant give the
same degree-58 norm, whose irreducible factorization is
\[
 \operatorname{Norm}(h_7)
 \doteq B^{16}E_3E_9E_{12}E_{18}.
\]
Direct quotient arithmetic identifies the \(B=0\) common root as \(X=0\),
and the \(E_{12}\) root as a point of \(R=S=0\). The unique roots over
\(E_3,E_9,E_{18}\) have \(X,S\ne0\). Hence the effective residual on
\(AS\ne0\) is
\[
 E_3(B)E_9(B)E_{18}(B)=0,
\]
giving 30 normalized values and 90 lifted algebraic points [MV/D].
Together with the \(F_3\) and \(F_6\) reductions, this finishes the
positive-dimensional factor-curve phase of the graph atlas.

EXP-128 computes the finite union in its squarefree quotient algebra [MV].
The seven retained factor appearances contain five distinct projected
factors of degrees
\[
  9,15,18,30,3,
\]
and their product has degree 75, not 102. The degree-9 factor is shared by
the \(F_3\) and \(F_7\) residuals; the degree-18 factor is shared by the
\(F_6\) and \(F_7\) residuals. Exact CRT idempotents verify the five-factor
direct product. Restricting \(h_7\) to the other curves shows that it is a
unit on the unique degree-15 \(F_3\) block and the unique degree-30 \(F_6\)
block. Its only failures there are the two shared blocks. The reciprocal
\(h_{36}\) section vanishes identically on \(F_7\), so a new section is
required on the complete degree-30 \(F_7\) divisor.

EXP-129 first verifies that the shared factors also have identical exact
\(X\)-classes in their two curve descriptions [MV]. Two finite-field probes
on each \(F_7\) block have augmented row rank 125. No single sampled row basis
covers all six probes, so one section does not suffice; a two-basis atlas
does. Both bases reconstruct in characteristic zero by the
same SCC/block-determinant method. If their primitive quotient sections are
\(k_1,k_2\), exact norm gcds and Bezout identities give
\[
 \operatorname{Norm}(k_2)\in
   \bigl(\Q[B]/(E_3)\bigr)^\times,
 \qquad
 \operatorname{Norm}(k_1)\in
   \bigl(\Q[B]/(E_9)\bigr)^\times
   \cap\bigl(\Q[B]/(E_{18})\bigr)^\times.
\]
Thus their norm ideal is the unit ideal in every block of
\(\Q[B]/(E_3E_9E_{18})\). Combined with \mbox{EXP-124--128}, this proves that the
maximal-minor atlas covers the complete \(AS\ne0\) rational graph [MV/D].

EXP-130 treats the finite base locus as a coordinate algebra rather than as a
list of algebraic points [MV]. Both projection resultants have degree 117.
After separating their coordinate factor of multiplicity 27, their
squarefree principal-open factors have degrees
\[
  3,6,12,69.
\]
Factorwise subresultants reconstruct the same-point \(X\)-class on every
factor, proving that
\[
 K=\bigl(\Q[X,B]/(R,S)\bigr)[X^{-1}]
\]
is a reduced product of four fields of total dimension 90. Direct
specialization gives
\[
 \gcd(R(0,B),S(0,B))=B^{12},\qquad
 \gcd(R(X,0),S(X,0))=X^3.
\]
This places the discarded coordinate support at
\(X=B=0\), on the separate \(A=0\) boundary. The projection multiplicity is
not used as a local-length claim.

The EXP-124 section is a unit on the degree-69 block, while the
\mbox{EXP-125-\(h_{36}\)} and \mbox{EXP-129-atlas-1} sections generate the unit ideal on
degree 12. The first newly selected exact basis leaves one common quadratic
in \(K[Y]\) on each of the degree-3 and degree-6 blocks. Modular probes at
the roots of those quadratics have full augmented rank, but reconstruction
of the default targeted basis exceeded a 300-second time limit. Starting
instead from the EXP-125-\(F_3\) basis and replacing one row reduces the
largest determinant SCC to 33. Its characteristic-zero determinant has
the form
\[
 A^{86}H(X,B,Y),\qquad \deg_Y H=2,
\]
and agrees with a direct 125-by-125 determinant at an independent control
point [MV].

On both remaining fields, this section breaks the common quadratic. Exact extended Bezout identities in \(K[Y]\) and independent
\(Y\)-resultants certify the final pairs. Multiplication matrix norms provide
an additional exact control and show that the
final pairs generate the unit ideal [MV]. Hence the finite maximal-minor atlas
covers \(V(R,S)\cap D(X)\) uniformly in \(Y=A^2C\). Combining the selected
EXP-123 chart, the EXP-129 graph atlas, and the EXP-130 base-locus atlas
exhausts every case with \(A\ne0\) [D].

EXP-131 returns to the original 302-by-125 augmented matrix before any
division by \(A\) and specializes directly at \(A=0,d=1\) [MV]. Two exact
row bases give
\[
...
and
\[
 \Delta_2=\kappa_2 B^{95}(109375B^6-110592)
 (21875B^6-4800B^3-24576),
\]
with \(\kappa_1,\kappa_2\in\Q^\times\); both are independent of \(C\).
For the squarefree divisors
\[
 f=(5B+4)(25B^2-20B+16),\qquad
 h=B(109375B^6-110592)(21875B^6-4800B^3-24576),
\]
exact extended Euclid gives
\[
\begin{aligned}
1={}&\left(
...
\end{aligned}
\]
Thus the two augmented minors generate the unit ideal in \(\Q[B,C]\), so
the complete \(A=0,d=1\) plane is covered [MV/D]. EXP-118 covers \(d=0\),
and EXP-123/129/130 cover \(A\ne0,d=1\). Therefore the complete
four-coefficient restriction
\(\{(0,1),(0,5),(1,0),(2,9)\}\) is closed [D]. The 24-parameter core,
complete 51-parameter family, \((72,108)\) case, planar degree floor, and
\(JC(2)\) remain open.

EXP-132 next adjoins \(T=\varepsilon_{(2,8)}\) and works on the same original
302-by-125 augmented matrix at \(A=0,d=1\) [MV]. Three normalized maximal
minors are
\[
\begin{aligned}
P(B)&=(5B+4)^3(25B^2-20B+16)^3,\\
Q(B,C,T)&=B^{95}\bigl(4785156250B^{12}+9765625B^{11}CT
 -1050000000B^9\\
&\hspace{38mm}{}-10214400000B^6+1061683200B^3+5435817984\bigr),\\
R(B,C)&=B^{105}C.
\end{aligned}
\]
Consequently
\[
 Q-9765625\,BTR=Q_0(B),\qquad \gcd(P,Q_0)=1,
\]
and the extended-Euclid computation gives explicit
\(u,v\in\Q[B]\) satisfying
\[
 uP+v\bigl(Q-9765625\,BTR\bigr)=1.
\]
Thus \((P,Q,R)=\Q[B,C,T]\), and the complete direct \(A=0,d=1\)
boundary of the five-coefficient restriction
\(\{(0,1),(0,5),(1,0),(2,9),(2,8)\}\) is closed [MV/D]. The primary
section is exactly \(T\)-inert; the alternative is affine in \(T\). The
final basis has 106 singleton cyclic components and a monomial
determinant. In contrast, the inherited EXP-123 shared basis and EXP-124
alternative basis both vanish identically on this direct specialization,
so neither inherited section suffices.

There is a useful module-theoretic reading: the augmented maximal minors are
the zeroth Fitting ideal of the cokernel, and the certificate above proves
that this specialized presentation is everywhere surjective. Recent work on
multivariate polynomial matrices gives Smith-form equivalence criteria for a
structured class with triangular highest determinant and extensions to
rectangular or rank-deficient matrices \cite{LuRuanWangXiao2026}. We have
not established those structural hypotheses for the GGHV augmented matrix,
so that theory is a comparison lens and a possible source of local reduction
lemmas, not a shortcut or an additional closure claim. The \(A\ne0,d=1\)
fifth-direction atlas is partly lifted: EXP-135 retains the EXP-124 section
exactly, while the two EXP-129 residual sections and EXP-130 finite-base
section remain. The transverse \(d=0\) kernel quotient also remains open;
hence the complete five-coefficient restriction, 24-parameter core, full
family, \((72,108)\), the planar degree floor, and \(JC(2)\) remain open.

\begin{proposition}[Ambient transverse inertness of the EXP-124 section]
\label{prop:exp135-inert}
Let
\[
 H(A,B,C)=I+(A-1)K_{(0,1)}+B K_{(0,5)}+C K_{(2,9)}
\]
be the normalized 33-by-33 cyclic core of the EXP-124 section, and
write \(K_T=K_{(2,8)}\). Then
\[
 \det\!\bigl(H(A,B,C)+T K_T\bigr)=\det H(A,B,C)
 \quad\text{in }\Q[A,B,C,T].
\]
The remaining 86 singleton blocks are also \(T\)-independent, so the complete
selected normalized maximal minor is unchanged by \((2,8)\).
\end{proposition}

\begin{proof}
Exact row reduction gives
\(\operatorname{rank}K_T=7\),
\(\operatorname{rank}K_T^2=3\), and \(K_T^3=0\). Fix a rank factorization
\(K_T=UV^{\mathsf T}\). The ranks of the four coefficient matrices in
\((A,B,C,T)\) are respectively \((25,24,6,7)\), so these are separate degree
bounds for the determinant difference. At each point of the tensor grid
\[
 A\in\{1,\ldots,26\},\qquad B\in\{0,\ldots,24\},\qquad
 C\in\{0,\ldots,6\},
\]
and at each of 30 distinct primes, exact modular elimination finds \(H\)
invertible and verifies
\[
 \bigl(V^{\mathsf T}H^{-1}U\bigr)^2=0.
\]
The matrix determinant lemma therefore makes
\(\det(I_7+T V^{\mathsf T}H^{-1}U)=1\) as a polynomial in \(T\) at every
grid point. Tensor interpolation proves that every coefficient of the
cleared determinant difference is zero modulo every listed prime.

For the characteristic-zero lift, the common entry denominator is
\(D=5{,}241{,}600\). The product of the rowwise coefficient \(\ell^1\)-norms
of \(D(H+T K_T)\) bounds the determinant coefficient \(\ell^1\)-norm.
Including the subtraction at \(T=0\) gives an explicit 885-bit absolute
coefficient bound. The 30-prime product has 897 bits and exceeds twice that
bound. A nonzero integer coefficient cannot therefore vanish modulo their
product. Every coefficient is zero, proving the identity over \(\Q\) [MV/D].
The exact singleton calculation is EXP-134.
\end{proof}

The proof changes the computational viewpoint. It avoids both a dense
33-by-33 symbolic determinant and graph-function-field inversion: rank gives
the interpolation degrees, low-rank determinant and Woodbury lemmas control
each finite evaluation, and a height bound turns modular vanishing into an
exact rational identity. The same route applies to the EXP-129 section, which is
observed to be inert; sections that are affine or quadratic in \(T\) require their
actual \(T\)-coefficients, since inertness is not inherited.

\emph{Beyond degree 125.} The chain $(8,40)/(8,28)/(11/4,7)$, of maximal degree $144$,
is already excluded in the published literature: the closing remark of \cite{GGV2} states
that the corner data $B_0 = (8,28)$, $B_1 = (8,40)$ force $A_0' = (8,4)$, which is not a
possible last lower corner (EXP-082). Of the other twenty-three configurations with maximal
degree in $[125,150]$ (EXP-083), four share a corner with the cases of that remark but fail its
hypotheses, so the remark does not exclude them (EXP-094). None of the twenty-three is decided
by these sources; for sixteen of them the last lower corner $A_0'$ is not printed in the
literature and has to be derived before membership in the excluded family $\wp(n', n'-1)$ can
be tested (EXP-083).
...
occur in the plane [MV], a conclusion consistent with the
$\mathbb{G}_m$-equivariant rigidity theorem of the companion paper \cite{SLfoundational},
which forbids the torus-graded planar
...
A search of the literature (the sources of Section~\ref{sec:frontier}) found no prior
statement of Theorems~\ref{thm:t1} or \ref{thm:t2}. For the equivariant rigidity theorem of
the companion paper \cite{SLfoundational}, Shaska \cite{Shaska2026}, in work submitted on
2026-07-22, independently proves the full all-signature classification.
...
elementary once seen, and some of these statements may be folklore; the primary sources
consulted, including the items that could not be verified, are listed in the repository.

\section{Open problems}

The computations above leave the following problems open: (1) lift the fifth direction
\((2,8)\) through the two EXP-129 residual sections and the EXP-130 finite-base section;
the complete-grid CRT argument of Proposition~\ref{prop:exp135-inert} applies wherever
explicit degree and height bounds are available; (2) construct the transverse \(d=0\)
kernel quotient and determine its Fitting ideal; (3) transport the intersection number
$21$ to the reduced system, which requires a complete account of the boundary divisors
through the swap, the Laurent cuts and the final inversion; and (4) derive a condition
beyond the first $D=72$ branch of the Newton-resolution list, which retains the GGHV
branch; without such a condition, Newton resolution does not exclude the pair. The
properness reformulation ($JC(2)$ holds iff every planar Keller map has empty Jelonek set;
exact certificates in EXP-014) provides an independent cross-check.

\section*{Data and code availability}

{\raggedright
All computations are exact and reproducible from the repository
\url{https://github.com/fsantibanezleal/CAOS_RESEARCH}. An identifier EXP-$nnn$ denotes
one archived record in the experiment folders of the problem,
\path{problems/algebraic-geometry/jacobian-conjecture/experiments/}; each record states its
question, method and result with its independent checks, together with the scripts and
outputs of its computations, and the larger computations list SHA-256 hashes of their result
files. Every [MV] claim has an archived script and output.\par}

\small
\begin{thebibliography}{99}
\bibitem{Alpoge2026} L. Alp\"oge, announcement of the counterexample, X (@\_\_alpoge\_\_),
2026-07-20; map produced with Claude Fable 5 (Anthropic); question posed by Akhil. % voice-ok: external attribution, as the announcement states it
...
arXiv:1401.1784.
\bibitem{GGV2} J. A. Guccione, J. J. Guccione, C. Valqui, The two-dimensional Jacobian
conjecture and the lower side of the Newton polygon, arXiv:1605.09430, 2016.
...
\bibitem{LuRuanWangXiao2026} D. Lu, Y. Ruan, D. Wang, F. Xiao,
Matrix equivalence to Smith normal form: new theoretical results for
multivariate polynomial matrices, arXiv:2605.09286v1, 2026.
...
\bibitem{SLfoundational} F. Santib\'a\~nez-Leal, The Jacobian conjecture after the
three-dimensional counterexample: exact validation, a general seed family, the geometry of
non-properness, and two-dimensional equivariant rigidity, preprint, 2026.
\href{https://doi.org/10.5281/zenodo.21503365}{doi:10.5281/zenodo.21503365}.
\end{thebibliography}
\normalsize
