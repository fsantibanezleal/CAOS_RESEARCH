...
\fancyhead[R]{\small Planar Keller Maps \textperiodcentered\ v0.21}
...
Paper B of a three-paper record \textperiodcentered\ version 0.21 \textperiodcentered\ 2026-07-29\\
...
with weights \((7,3,9)\) after shifting the forced coefficient. These factors
define the next residual charts. On their \(d=0\) boundary, the tautological
kernel \([P,P]=0\) can be quotiented exactly. One quotient determinant then
factors into \(b^{32}\), one five-term weighted factor, and five binomials.
For \(b\ne0\), \(z=a^3/b^7\) reduces its residual to nine distinct invariant
points. These are finite alternative-chart targets, not a closure claim.
...
points. Alternative 124-column charts are still required there. This does
not close the 24-parameter core, the full \((72,108)\) family, or \(JC(2)\).
...
Priorities, from the standing routes evaluation: (1) cover the finite
EXP-117 quotient residual: the divisor \(b=0\), five rational
\(z=a^3/b^7\) values, and the squarefree quartic fibre; (2) close the proper
intersections left on the three EXP-115 \(d\ne0\) components; (3) retain the exact EXP-108 lift through
\((0,6)\) as a bounded chart control rather than a terminating programme;
(4) reopen the intersection-$21$ transport only
