...
\date{2026-08-01\\[2pt]\normalsize Version 0.32}

\pagestyle{fancy}
\fancyhf{}
\fancyhead[L]{\small CAOS Research Preprint}
\fancyhead[R]{\small Planar Keller Maps \textperiodcentered\ v0.32}
...
Paper B of a three-paper record \textperiodcentered\ version 0.32 \textperiodcentered\ 2026-08-01\\
...
declared four-parameter restriction is covered. Returning to the original
...
previous \(d=0\) quotient cover, this closes the complete declared
...
one exact row combination, two depend only on \(B\) and are coprime, with a
persisted characteristic-zero Bezout certificate [MV/D]. The inherited
EXP-123 and EXP-124 bases vanish identically on this specialization, so the
successful certificate required a new SCC-guided basis rather than a
parameter substitution into the old atlas. EXP-135 then proves that the
accepted EXP-124 section is unchanged by \((2,8)\) on its complete normalized
...
certify \(\det(H+T K_{(2,8)})=\det(H)\) [MV/D]. Hence the old
\(F_3F_6F_7\) divisor is retained for this section. The other residual
...
At this stage it is not a four-parameter chart cover: alternative minors must
cover the rational graph, the finite base locus, and the separate divisor
\(A=0\). Sections below record the subsequent graph and base-locus closure.
...
EXP-127 applies the same dictionary to the distinct persisted \(F_7\) basis
...
the \(F_3\) and \(F_7\) ledgers; the degree-18 factor is shared by the
\(F_6\) and \(F_7\) ledgers. Exact CRT idempotents verify the five-factor
...
on each \(F_7\) block have augmented row rank 125. No one sampled row basis
covers all six probes, refuting the single-section prediction, but a
two-basis atlas does. Both bases reconstruct in characteristic zero by the
...
\(\Q[B]/(E_3E_9E_{18})\). Combined with EXP-124--128, this proves that the
...
EXP-125-\(h_{36}\) and EXP-129-atlas-1 sections generate the unit ideal on
...
of the default targeted basis reaches the declared time gate. A structural
redirect starts from the EXP-125-\(F_3\) basis, replaces one row, and reduces
the largest determinant SCC to 33. Its characteristic-zero determinant has
the form
\[
...
On both remaining fields, the structural section breaks the persisted common
quadratic. Exact extended Bezout identities in \(K[Y]\) and independent
...
and EXP-123/129/130 cover \(A\ne0,d=1\). Therefore the complete declared
four-coefficient restriction
...
refuting the declared prediction that an inherited section would suffice.
...
\]
be the normalized 33-by-33 cyclic core of the accepted EXP-124 section, and
...
\[
 ...
 C\in\{0,\ldots,6\},
\]
and at each of 30 persisted primes, exact modular elimination finds \(H\)
...
exact rational identity. This route will be reused on the observed-inert
EXP-129 section; affine or quadratic sections must reconstruct their actual
\(T\)-coefficients rather than inherit inertness.
...
Priorities, from the standing routes evaluation: (1) preserve the exact
EXP-118/123/129--132 certificates, including the direct \(A=0,d=1\)
five-coefficient unit-ideal identity, and EXP-135's retained EXP-124 divisor;
(2) lift the fifth direction \((2,8)\) through the two EXP-129 residual
sections and EXP-130 finite-base section, reusing complete-grid CRT
certificates only with explicit degree and height bounds; (3) rebuild the transverse \(d=0\)
kernel quotient and test its Fitting ideal independently; (4) retain
the complete EXP-118--121 \(T_B\)
cover as a regression gate rather than adding redundant slice charts; (5) retain the
exact EXP-108 lift through \((0,6)\) as a bounded chart control rather than
a terminating programme; (6) reopen the intersection-$21$ transport only
after constructing a complete boundary-divisor ledger through the swap,
Laurent cuts, and final inversion; (7) pursue Newton resolution only after deriving a
condition beyond the already-retained first $D=72$ branch; and (8) retain the
