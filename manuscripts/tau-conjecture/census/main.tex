...
prices the gap: the scan is conclusive on every candidate with $|c| \le 395$,
and window-limited only on candidates with a large trailing coefficient. For an
additive last gate the trailing coefficient is determined by the constant terms
of $v$ and $b$, which the stored state already holds, so the window-limited
fraction of the search space is measurable exactly rather than by estimate.
