...
\fancyhead[R]{\small Tau-Conjecture Census \textperiodcentered\ v0.01}
...
First paper of the tau-conjecture record \textperiodcentered\ version 0.01 \textperiodcentered\ 2026-08-01\\
DOI \href{https://doi.org/10.5281/zenodo.21753438}{10.5281/zenodo.21753438} (concept DOI, resolves to the latest version) \textperiodcentered\ this version: \href{https://doi.org/10.5281/zenodo.21753439}{10.5281/zenodo.21753439}\\
...
roots is $6$ gates, and of $6$ roots is $8$ or $9$ gates; the growth function has
plateaus at $\tau = 4$ and $\tau = 7$, so an extra gate does not always buy an extra
root. The censuses are decision-complete: depth $6$ required the exact construction of
...
computer-algebra root counter on $284$ polynomials. We further prove two elementary
...
reproducible from committed code with hypotheses declared before each run; two of our
...
roots it lies in $[8, 9]$ (the census excludes $7$; a $9$-gate witness exists).
...
criterion. Two committed predictions were refuted by the machine: we predicted
$\zmax(6) = 4$ (the census returned $5$; Section~\ref{sec:census}), and we predicted
zero tower yield off the fixed-point series (a second series exists;
Section~\ref{sec:family}). Both refutations are reported alongside what they taught.
...
$k=2{:}\,2$; $k=3{:}\,3$; $k=4{:}\,5$; $k=5{:}\,6$; and for $k=6$ it lies in
$\{8, 9\}$.
\end{theorem}

The $k = 6$ upper bound is the $9$-gate witness $q(q-2)(q-6)$ with $q = x^2 - x$
(root set $\{-2, -1, 0, 1, 2, 3\}$); the lower bound is
Theorem~\ref{thm:census}. Representative optimal witnesses, replayed gate by gate in
...
(i) close the $\{8, 9\}$ window of Theorem~\ref{thm:thresholds}: the $8$-gate
decision is finite and SAT-shaped (in the style of exact SLP synthesis over GF(2)
...
compute for every result in this paper is under two hours on one desktop CPU
