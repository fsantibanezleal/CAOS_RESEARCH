...
\newcommand{\versiondoi}{10.5281/zenodo.22823624}
\newcommand{\conceptdoi}{10.5281/zenodo.21997377}
...
\title{Curvilinear Geometry, Primary Structure, and Gr\"obner\\
Degenerations of Conductor Fiber Cones\\ in a Huneke--Wiegand Family}
...
proof is deductive from previously proved offset-basis and Cohen--Macaulay input theorems. We
further give the complete reduced Gr\"obner basis for graded reverse lexicographic order with
$X_0$ last. Its degree profile is
\[
 (50p^2-17p,\;5p-1,\;p-2)
\]
in degrees two, three, and four, with no later elements. No minimal leading monomial contains
$X_0$, yielding a flat Cohen--Macaulay monomial degeneration. Exact parameter sweeps, an
independently encoded clique-based recomputation, and an all-parameter Presburger boundary
certificate validate the formulas and run negative controls, but do not replace the deductive
arguments.
\end{abstract}
...
explicit infinite family of Huneke--Wiegand counterexamples. The family was constructed in
\cite{SantibanezLeal2026} after Son Pham's discovery of a first public counterexample
\cite{Pham2026}, which Craig Huneke independently verified; the discovery priority and the
present contribution are distinct. The same earlier work determines the conductor fiber cone,
its complete degreewise offset bases, its
...
Throughout, derived theorem statements are marked [D] by convention, while finite exact
computations and solver outcomes are machine-verified [MV]. No [MV] result is used as an
extrapolation principle.

\section{The conductor family and imported input}
...
\begin{proposition}[Imported offset-basis input]\label{prop:input}
...
and offset-congruence theorems. The present paper does not infer it from the finite computations
below. Its exact all-parameter component step retains the solver/encoding boundary disclosed in
...
its zero ideal is not primary. This example shows that the associated-prime argument cannot be
replaced by a radical-only inference.
\end{remark}
...
The symbolic proofs above use the imported results in Proposition~\ref{prop:input}. Computation is
used to validate their translation into the new parametrization and to exercise negative controls.
The primary-structure sweep computes truncated sumsets directly from $G_p$ for every
$p=4,\ldots,300$. An independent recomputation starts from the five disjoint Artinian layers. The
Gr\"obner sweep independently checks all families listed in Theorem~\ref{thm:groebner}, canonical
tails, divisor minimality, Hilbert values, and controls at the same 297 parameters. A separately
encoded clique-based recomputation enumerates canonical pairs and reconstructs factorization
complexes through degree six at \mbox{$p=4,5,6,17,73,151,300$}.

\begin{center}
\begin{tabular}{@{}lll@{}}
\toprule
computation & coverage & SHA-256 aggregate\\
\midrule
primary sweep & all $p=4,\ldots,300$ & \texttt{f3373f4f58287fd3...f9d6b39b6}\\
primary recomputation & all rows; six detailed rebuilds & \texttt{84c00be8ff64002...f5b6c8f143e}\\
Gr\"obner sweep & all $p=4,\ldots,300$ & \texttt{63af8f734afc8c05...9e4365a218}\\
clique recomputation & seven detailed rebuilds & \texttt{401c4807cc0a29a6...586766e41373}\\
Presburger boundary & all integers $p\geq4$ & \texttt{10c66bbcaa56108f...e046f9dd5fa}\\
\bottomrule
\end{tabular}
\end{center}

The negative controls delete offset $1$, perturb the truncation and nilpotency exponents, corrupt
the degree-three hole, omit a radical coordinate, conflate local and arithmetic Gorensteinness,
erase the differential characteristic split, and remove the no-embedded-primes premise. Every
mutation is rejected. Exact scripts, artifacts, premise hashes, and proofs are archived in the
public repository as the computation records \path{EXP-025-curvilinear-primary-structure} and
\path{EXP-026-grevlex-staircase}, in the folder
\path{problems/commutative-algebra/huneke-wiegand/experiments/}.
...
Proposition~\ref{prop:differentials}. A defect in an imported input theorem would propagate even
if both implementations agreed. The Presburger boundary certificate uses Z3 4.16.0 and does not
...
With the primary decomposition and the reduced Gr\"obner basis determined, one major algebraic
