...
\newcommand{\versiondoi}{10.5281/zenodo.22859408}
...
\fancyhead[R]{\small Quadratic Integral Two-Torsion \textperiodcentered\ \docversion}
...
Independent finite computations check the signed identities and complete-sector
parity certificates, preserving all tested sources in lossless archives.
A failed local search yields an exact locality obstruction.
For a semantic row projection retaining four of the six recurring row types,
we give support-one integral sources that kill two endpoint triangle rows for
every $p\geq8$; the proof is a direct face classification before contraction.
...
were developed in \cite{CAOSMain}. We make
...
presentation identified in \cite{CAOSMain}; it neither repeats nor claims
priority for the discovery of the original public Huneke--Wiegand
counterexample.
...
Section~\ref{sec:projection} gives a separate constructive comparison with a
four-type semantic row projection and identifies two uniformly vanishing
endpoint rows.
These are lower-bound and partial-decomposition results, not the whole quotient.

\paragraph{Deductive proofs and finite certificates.}
The generic parity, independence and splitting arguments below are deductive.
An independent full-sector recomputation also certifies all 27 triangles at
the five parameters $p=8,\ldots,12$.
...
\section{A uniform semantic projection identity}\label{sec:projection}

The following projection is defined on the full original target, before any
unit contraction or component selection. For an exterior set $E$, write
\[
 \nu(E)=(|E\cap L_0|-p,|E\cap L_1|-p,
          |E\cap H_0|,\ldots,|E\cap H_7|).
\]
The six recurring row types used here are
\begin{center}\small
\begin{tabular}{cll}
\toprule
Type & Target family & $\nu(E)$\\
\midrule
$R_0$ & $D[E;\mathsf A_d]$ & $(-2,-3,1,0,1,0,0,0,0,0)$\\
$R_1$ & $D[E;\mathsf A_d]$ & $(-3,-2,2,0,0,0,0,0,0,0)$\\
$R_2$ & $D[E;\mathsf B_d]$ & $(-1,-4,1,0,1,0,0,0,0,0)$\\
$R_3$ & $D[E;\mathsf B_d]$ & $(-2,-3,2,0,0,0,0,0,0,0)$\\
$R_4$ & $K[E;b]$, $b\in\mathrm C_0$ & $(-2,-2,1,0,0,0,0,0,0,0)$\\
$R_5$ & $K[E;b]$, $b\in\mathrm C_2$ & $(-1,-3,1,0,0,0,0,0,0,0)$\\
\bottomrule
\end{tabular}
\end{center}
Let $\Pi_{58}$ be the coordinate projection that retains all rows of types
$R_1,R_3,R_4,R_5$ and discards every other target row. The numeral is only a
stable name for this four-type mask; it is not a modulus.

\begin{theorem}[{[D]} Support-one endpoint sources]\label{thm:mask58-endpoints}
For $r\in\{1,2\}$, put
\begin{align*}
 E_{p,r}&=(L\setminus\{p-r,3p,3p+r\})\cup\{6p,10p\},\\
 s_{p,r}&=S[E_{p,r};p-2].
\end{align*}
Then, for every $p\geq8$,
\begin{equation}\label{eq:mask58-endpoints}
 \Pi_{58}M_ps_{p,r}=-x_{(0,r,p-2-r)}.
\end{equation}
Thus the two endpoint triangle rows vanish integrally in
$\coker(\Pi_{58}M_p)$, each with a one-column source of coefficient height one.
\end{theorem}
\begin{proof}
The exterior $E_{p,r}$ contains $p-1$ first-low generators, $p-3$
second-low generators, and the two displayed high generators, hence has
size $2p-2$ as required.

Write $a=p-2$. If $v\in L_0$, then $\mu(v,a)$ is nonzero exactly when
$v+a>p$, or $v\geq3$. Deleting such a present generator gives normalized
counts $(-2,-3,1,0,1,0,0,0,0,0)$, hence an $R_0$ row. There are $p-3$
of them because $p-r$ is omitted. If $v\in L_1$, then $\mu(v,a)$ is
nonzero exactly when $v+a\geq4p-1$, or $v\geq3p+1$. The source already
omits $3p$ and $3p+r$, so all its $p-3$ remaining second-low deletions
give $R_2$ rows, with normalized counts
$(-1,-4,1,0,1,0,0,0,0,0)$.

Deleting $6p$ would give coefficient $7p-2$, which belongs to the
degree-one interval $H_0$ and not to the degree-two target basis, so that
face contributes nothing. Deleting $10p$ gives coefficient $11p-2\in
\mathrm C_2$ and normalized counts
$(-1,-3,1,0,0,0,0,0,0,0)$, hence type $R_5$. The generator $10p$ is
last in a $2p-2$ element exterior set, at zero-based position $2p-3$,
so its sign is $-1$. Its face is precisely the row
$x_{(0,r,p-2-r)}$ by the triangle-row definition.

The complete nonzero boundary therefore consists of $p-3$ rows of type
$R_0$, $p-3$ rows of type $R_2$, and the single negative $R_5$ row in
\eqref{eq:mask58-endpoints}. The projection discards the first two types
and retains the last one.
\end{proof}

The identity is stronger than a finite rank observation because it is proved
in the full original presentation before contraction. It is also narrower
than an identification of presentations: no uniform isomorphism between this
semantic projection and a persistent isolated component is asserted. At the
locked parameter $p=11$, the frozen component contains unique matching source
columns 210 and 308, whose retained entries are respectively $-1$ in rows
12559 and 12560 [MV]. This exact finite agreement does not extend the scope of
Theorem~\ref{thm:mask58-endpoints} to a uniform component reconstruction.
...
\subsection{A radius-two locality obstruction}

Computation EXP-058 explores original source columns
...
radius-two span excludes $2\eta_8$ even over $\mathbb Q$ [MV].
...
mutation check verify the certificates independently. The obstruction is
computed only at $p=8$; the cases $p=9,10$ were not computed.
...
\subsection{Verification runs and negative controls}

For Theorem~\ref{thm:basis}, EXP-059 checks every unit potential at
$p=8,\ldots,16$, giving 525 chains, and four fixed endpoint pairs
at each $p=17,\ldots,100$, giving 336 more [MV]. All 861 full original
boundaries agree with the seven-row formula. The primary computation uses
both left-to-right and independently encoded right-to-left differentials.
A separate coefficient-first source reconstruction and a bitset differential
independently recheck all 861 cases; the latter also crosschecks the literal
differential on the 525 small cases. Source support ranges from 9 to 197 and
boundary support from 3 to 7 in this computation. The integral completeness
...
An independent re-implementation reconstructs all source labels and coefficients,
...
Wrong signs and single-coefficient changes are rejected. Two variants of
$V_p$ fail uniformly. Omitting the $\delta_{02}/Q_2$
correction gives $V_{\mathrm{omit}}=V_p+4D$ and
\[
 M_pV_{\mathrm{omit}}-2\eta_p=4(x_{01}+e(2;0,1))\ne0.
\]
Reversing, in addition, the sign of $F_1$ gives
$V_{\mathrm{early}}=V_{\mathrm{omit}}-4P(F_1)$ and
\[
 M_pV_{\mathrm{early}}-2\eta_p=4(e(2;0,1)-x_{01})\ne0.
\]
The rows are distinct, so both failures hold uniformly.

Each computation and its independent recomputation runs in under 60
seconds in one CPU process with at most 1~GiB of private memory. These
verification runs perform no new HNF, Smith-form or full ambient-basis
computation. No old labelled HNF source, including the one at $p=11$, is
an input, and the check of the new formula at $p=11$ is not an untouched
held-out test.
...
element is checked at $p\leq12$; larger parameters use fixed first,
middle and last basis indices, together with the additional free $d=2$
...
include $10p-3$. The checks include the free diagonal cases $(u,r)=(2,2)$
and $(4,4)$ at $p=8$, where the two individual pairings are both one and
cancel.

The independent recomputation does not use the potential formulas of the
primary computation.
...
All 20 independent negative controls pass. They include removal of the
...
primary computation and the independent recomputation each run in under
120 seconds in one process with at most 1~GiB of memory. These finite
checks test the
...
The primary computation also checks seven complete small-parameter pairing
matrices, 93 count identities at $p=8,\ldots,100$, and the five original
...
The sources were selected before the computation; the count theorem at
$p=100$ does not mean that every source at that parameter was computed.

The independent recomputation reconstructs all 70 signed sources, all five
transfers, the pairings, counts and mutation controls without importing
formulas from the primary computation. For every one of the 27 triangles at
...
\begin{tabular}{lr}
\toprule
Full-sector certificate quantity & Count\\
\midrule
...
not merely the potential chains constructed by the primary computation.
...
the check also multiplies the actually altered sources. Duplicate
and mirrored edge choices give singular pairing matrices. The independent
recomputation replays byte for byte.

Observed source support ranges from 38 to 13,594, with coefficient height
two throughout these computations. These are finite observations, whereas
\eqref{eq:template-support} proves the ten-to-twelve-row functional bound
uniformly. Both computations run in under 120 seconds in one process
with at most 1~GiB of memory. No dense full ambient matrix, integral
...
\subsection{Uniform endpoint projection certificate}

Computation EXP-066 evaluates the two identities of
Theorem~\ref{thm:mask58-endpoints} for every $p=8,\ldots,300$, giving 586
exact endpoint checks, and rejects 2,930 altered sources, target rows and
projection masks [MV]. An independent implementation reconstructs the interval
types and normalized counts in reverse parameter order and recovers the same
two locked $p=11$ component columns. The all-parameter conclusion comes from
the preceding interval inequalities and face count, not from this finite sweep.
...
Theorem~\ref{thm:mask58-endpoints} gives explicit sources for two endpoint
rows after one semantic projection. It does not prove independence or an upper
bound for the other projected triangle rows, nor that this projection agrees
uniformly with the earlier persistent isolated component.

The mathematical premise imported from the earlier family is the integral
coefficient presentation itself. Independent encodings test for implementation,
...
\appendix
\section{Computation records and archived sources}

All computations are archived in the public \Repo\ repository. The EXP
identifiers used in this paper denote the computation records in its folder
\path{problems/commutative-algebra/huneke-wiegand/experiments/}; each record
contains the proofs, source code and exact artifacts of that computation.
The following table links each result to its records.

\begin{center}\small
\begin{tabular}{>{\raggedright\arraybackslash}p{0.43\textwidth}>{\raggedright\arraybackslash}p{0.46\textwidth}}
\toprule
Result & Computation record\\
\midrule
...
Theorem~\ref{thm:basis}, 861-chain verification &
...
Proposition~\ref{prop:eta-edge} and complete triangle certificates &
\Exp{EXP-062-triangle-torsion-family}{EXP-062}\\[3pt]
Theorem~\ref{thm:mask58-endpoints}, locked holdout and reverse-order audit &
\Exp{EXP-065-dual-guided-endpoint-decoder}{EXP-065},
\Exp{EXP-066-uniform-endpoint-face-collapse}{EXP-066}\\
\bottomrule
\end{tabular}
\end{center}

The full EXP-060 source archive retains every original exterior label and
integer coefficient for all 18 parameters. The EXP-062 archive
...
byte counts, parameter counts, and per-source hashes. The independent
recomputations decompress the archives and reconstruct all labels; no
mathematical information is discarded by compression.
...
EXP-060 independent recomputation & \multicolumn{2}{c}{exact JSON certificate} & \texttt{3bcf3cabe5b0330f}\dots\\
EXP-062 source archive & 17,253,771 & 252,812 & \texttt{8475ffb305c1567e}\dots\\
EXP-062 independent recomputation & \multicolumn{2}{c}{exact JSON certificate} & \texttt{854405294fab1d88}\dots\\
\bottomrule
\end{tabular}
\end{center}
...
hashes in the manifest and in the independent-recomputation certificate. The
accompanying tests also verify deterministic gzip replay, all stored source
hashes, and the original $b_p$ source transfer at $p=8$.

\begin{thebibliography}{9}
\bibitem{CAOSMain}
F.~Santib\'a\~nez-Leal,
\emph{Frobenius Minimality, Minimum-Layer Uniqueness, and an Infinite Family
of Huneke--Wiegand Counterexamples}, CAOS Research preprint,
2026. \href{https://doi.org/10.5281/zenodo.21763582}{doi:10.5281/zenodo.21763582}.
