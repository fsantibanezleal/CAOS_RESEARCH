...
\newcommand{\versiondoi}{10.5281/zenodo.22835200}
\newcommand{\conceptdoi}{10.5281/zenodo.21554258}
\newcommand{\docrepo}{github.com/fsantibanezleal/CAOS\_RESEARCH}

\newcommand{\headerblock}{%
\begin{center}
\fbox{\parbox{0.94\textwidth}{\small\raggedright
\textbf{\doctype} (independent verification) \quad$\cdot$\quad
\docversion, \docdate \quad$\cdot$\quad Licence CC BY 4.0\\[2pt]
CAOS Research open-problems programme \quad$\cdot$\quad problem \texttt{unsplittable-flow-cost}\\[2pt]
...
\thanks{CAOS open-research programme, Santiago, Chile. Code, experiment records
and artifacts: \url{https://github.com/fsantibanezleal/CAOS_RESEARCH}, folder
...
the conjecture fails by \emph{one} unit at $\dmax = 15$, so the open question whether
...
replication; items (ii) and (iv)--(vi) are, to our knowledge, new. The
two-terminal case remains open, and we show why a natural argument for it
fails.
\end{abstract}
...
rejected by the instance constructor. It was calibrated first on instances whose
...
$c = \tfrac17(2,\allowbreak 3,\allowbreak 0,\allowbreak 2,\allowbreak 0,\allowbreak 0,\allowbreak 0,\allowbreak 0,\allowbreak 0)$, which is the published cost vector
normalised.
\end{proposition}
...
\begin{remark}[an invalid argument for two terminals]
It is tempting to conclude that no counterexample exists with at most two
terminals, on the grounds that a conflict graph on at most two nodes has no odd
cycle. That argument is invalid: a terminal may have many path choices, so the
conflict graph need not have two nodes, and the step from an integral stable-set
polytope to the absence of a separating nonnegative cost vector is unproved. The
two-terminal case is open.
\end{remark}
...
one. This is evidence over a bounded box, not a proof.
...
every instance admits a cost-good routing within $x + \alpha\dmax$, the bounds
known after this work are $\alpha^\star \ge 16/15$ and, for planar instances,
...
July 2026, attributed to D.\ Rybin working with a large language model, and was % voice-ok
obtained as a file bundle, which we archived with SHA-256 hashes. Our
contributions are the independent exact verification, the quantitative
...
single-terminal theorem and the two-terminal characterisation. We make no claim
about priority or attribution. Every computation
is exact and reproducible from the released code. The computations are archived
as the experiment records EXP-001 to EXP-003 in the repository folder
\path{problems/optimization-geometry/unsplittable-flow-cost/experiments/}; each
record holds the code, the artifacts and the result.
