...
while PENDINGPAIRS of its 1{,}326 vertex pairs do (explicit witnesses). Finally, we prove that
...
two seconds), while PENDINGPAIRS of the $1{,}326$ vertex pairs admit a map whose star
condition fails exactly at the two relaxed vertices (explicit witnesses; the checker reports
exactly those two bad vertices).
\end{theorem}
...
Section~\ref{sec:compositions}), can absorb it, whereas PENDINGPAIRSWORD pairs can. For the
112-vertex graphs the single-vertex sweep is reported in the repository record (experiment
006); at the time of this version the sweep over their free vertices had established
PENDING112.
