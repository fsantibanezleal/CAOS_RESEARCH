...
`p=8,9,10,11`. Exact rational ranks plus first-Bockstein ranks show that every
even Smith factor has valuation one. Consequently EXP-063's two mask-58
endpoint relations are integral vanishings, not hidden doubles of order-four
classes. Every other triangle generates the complete mask-58 2-primary
subgroup; all triangles generate the complete mask-59/62 2-primary subgroups.
Mask 56 is trivial through p=10 and generated by `(2,3,4)` at p=11.

The independent audit recomputes 1,100 prime/rank pairs with opposite pivots,
checks 16 minimally sufficient Hadamard products, 16 mutations, two exponent
controls and all 16 triangle consequences. The theorem is exact but finite and
nonconstructive at the source-witness level. Uniform endpoint sources,
transformed duals, the complementary quotient and recurrence remain open. No
manuscript or Zenodo revision is triggered.

## Prior closed result: finite labelled triangle comparison, 2026-09-20

[EXP-063](../experiments/EXP-063-triangle-isolated-comparison/verdict.md)
proves an exact finite target-coordinate bridge at `p=8,9,10,11`. All 19
EXP-062 triangle rows survive the integral unit contraction literally and are
`R5` rows of the persistent component. Their mod-two quotient ranks in masks
59 and 62 are `3,4,5,7`, with no relations. Mask 58 has ranks `1,2,3,5`, and
its complete finite relation space is generated by the endpoint triangles
`(0,1,p-3)` and `(0,2,p-4)`. Mask 56 has ranks `0,0,0,1`.

An independent audit reconstructs every labelled component, verifies all 19
persisted `+/-2x_T` boundaries, enumerates every triangle subset in all 16 mask
cases, and passes 12 mutation controls. This is a finite mod-two comparison,
not an all-parameter integral kernel theorem. In particular, mod-two vanishing
does not exclude an order-two image from being twice an order-four class.
The next gate is a uniform integral witness/dual calculation for the two
endpoint classes, then the complementary quotient and only then recurrence.
No manuscript or Zenodo revision is triggered by the finite result.

## Prior closed result: quadratic two-torsion direct summand, 2026-09-05
...
The full quotient, a matching upper bound, free rank, additional torsion, and
