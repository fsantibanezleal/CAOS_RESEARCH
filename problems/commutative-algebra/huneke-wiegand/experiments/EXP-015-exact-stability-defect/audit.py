"""Independent exact audit for EXP-015; imports no experiment code."""
...
def reconstruct(p: int) -> dict[str, object]:
    ...
    expected_square = {8 * s + residue for residue in c} | seq(9 * s, stop)
    if square != expected_square:
        raise AssertionError(f"audit p={p}: square mismatch")
    t_values = {
        level * s + residue
...
def main() -> int:
    ...
    print(f"EXP-015 audit PASS aggregate={output['audit_aggregate']}")
    return 0
