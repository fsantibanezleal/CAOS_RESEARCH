...
Declared before the run (context/2026-08-25-confinement-lemma.md): "the fraction
will be LOW (most nine-gate values carry large constants), so the honest
headline stays windowed and the value of EXP-014 is that it prices the gap
...
construction. 6,400,000 candidates constructed.
...
The residual class is small and well characterized: degrees 8 (100), 9 (2) and
12 (20); 64 distinct |c| values from 512 to 38,220. A divisor refinement (a
seven-rooter's roots all divide c, and the least |product| of 7 distinct nonzero
integers is 144, rising to 1188 when one root escapes) eliminates only 2 of the
64 values, so it is not by itself enough to close the class.
...
1. A vectorized version carrying the low K coefficients reported ~1.5%
   ...
   found ZERO nonzero candidates divisible by x^20: every one of those 1.47% is
   the IDENTICALLY ZERO polynomial. Low coefficients alone cannot tell "deeply
   divisible" from "zero", and the vectorized instrument had no way to see it.
   This is the FIFTH appearance of the zero-object degeneracy in this problem.
2. A valuation-pair instrument (v, L) was written to remove the coefficient
   ...
   cancelling leads are common among these operands. It was kept only as a
...
roughly 4.2e12 candidates, 0.0019% is on the order of 1e8 candidates that the
window alone does not decide.

It does replace an unbounded caveat with a measured and structurally
characterized one. The honest form of the pending nine-gate statement is
therefore: emptiness holds outright for 99.998% of the additive search space,
and is windowed only on a residual of about 0.002% whose degrees and trailing
...
with |c| above the bound is affordable, and each survivor can then be root-counted
exactly over the divisors of its trailing coefficient.
