# EXP-075: degree-3 quadruple-support necessaries. Imports EXP-071's pipeline.
# t = 1. Run: run.py
...
def main():
    print("=" * 76, flush=True)
    print("EXP-075: degree-3 quadruple-support necessaries (t = 1)", flush=True)
    print("=" * 76, flush=True)
    t0 = time.time()
    ...
    if bad:
        ...
        sys.exit(1)
    # 2: the quadruple-support sweep, early-abort
    t0 = time.time()
    start = int(sys.argv[1]) if len(sys.argv) > 1 else 0
    trips = list(combinations(range(len(ops)), 4))
    hit = None
    for n_, (a_, b_, c_, d_) in enumerate(trips):
        if n_ < start:
            continue
        sup = (ops[a_], ops[b_], ops[c_], ops[d_])
        ...
        if (n_ + 1) % 200 == 0:
            print(f"  2: {n_ + 1}/{len(trips)} quadruple supports OK "
                  f"({time.time() - t0:.0f} s)  [resume index {n_ + 1}]",
                  flush=True)
    if hit:
        check("2: the quadruple-support sweep", True,
        ...
        check("2: the quadruple-support sweep", True,
              "ALL triple supports feasible: degree 3 open through quadruples; "
