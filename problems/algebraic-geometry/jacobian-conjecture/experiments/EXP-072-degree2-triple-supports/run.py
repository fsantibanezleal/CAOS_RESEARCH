# EXP-072: degree-2 triple-support necessaries. Imports EXP-071's pipeline.
# t = 1. Run: run.py
...
def main():
    print("=" * 76, flush=True)
    print("EXP-072: degree-2 triple-support necessaries (t = 1)", flush=True)
    print("=" * 76, flush=True)
    t0 = time.time()
    ...
    bad = [s for s in sample
           if not e71.deg2_subsystem_feasible(ctxs[PRIMES[0]], s)]
    ...
    for n_, (a_, b_, c_) in enumerate(trips):
        ...
        if not e71.deg2_subsystem_feasible(ctxs[PRIMES[0]], sup):
            if not e71.deg2_subsystem_feasible(ctxs[PRIMES[1]], sup):
                hit = sup
                break
    ...
    if hit:
        ...
        print("RESULT: ALL CHECKS PASS. (degree 2 closed at a triple support)",
              flush=True)
    else:
        check("2: the triple-support sweep", True,
              "ALL triple supports feasible: degree 2 open through triples; "
              "quadruples or the constructive path staged")
        print("RESULT: ALL CHECKS PASS. (degree 2 open through triples)",
              flush=True)
