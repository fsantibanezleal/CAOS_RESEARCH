...
def petersen_relaxed_vertices(g: Graph, relaxed: set[int], symmetry: bool = True) -> CNF:
    """Edge map into E(P) whose star condition may fail only at the vertices in `relaxed`."""
    inc = _stars(g)
    p = petersen()
    pedges = p.edges
    padj = [[j for j in range(15) if j != i and set(pedges[i]) & set(pedges[j])] for i in range(15)]
            ...
            for s in range(15):
                for t in range(15):
                    if s == t or t not in padj[s]:
                        f.add(-y[a, s], -y[b, t])
    if symmetry:
        e0 = next(e for e in range(len(g.edges)) if not (set(g.edges[e]) & relaxed))
        u = g.edges[e0][0]
        e1 = next(x for x in inc[u] if x != e0)
        f.add(y[e0, 0])
        f.add(y[e1, padj[0][0]])
