...
| NS-003 | EXP-001: build `openai/NavierStokesAndEuler` and run both Comparator challenges | validation | Navier-Stokes done, Euler build in cache repair | P1 | L | 6 h wall clock, 60 GB disk on E: | abandon if the mathlib cache plus one module smoke build exceeds 90 min or if peak RSS exceeds available RAM |
...
| NS-012 | Euler certificate build: repair the mathlib cache corrupted by the disk-deletion storm and rebuild | validation | doing | P2 | L | forced cache re-fetch 2 h, build 8 h | abandon below 5 GB free on E: |
