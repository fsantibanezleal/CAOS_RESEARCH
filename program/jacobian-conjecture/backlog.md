...
| JCB-038 | THE STAIRCASE TRANSPORT (the open core): the constant must travel the lower-left Newton boundary from the swallowed vertex (1,0) to the top corner. Route: (a) formalize the staircase as a sequence of weight systems (one per edge), each contributing a banded/multiplication class system (Theorems 1-3 apply edge by edge); (b) derive the transport recursion linking consecutive edges at each vertex; (c) show the recursion is over-determined; (d) meanwhile widen scans. This is the classical Moh staircase with new instruments | P5 | todo | 2026-07-22 | the residual open core |
| JCB-039 | Pair-level corner theory: both components cornered (tops c B^p, c' B^q); Abhyankar similarity in the diagonal picture; test whether the pair constraint adds anything the single-component staircase does not | P4 | todo | 2026-07-22 | complementary frame |
