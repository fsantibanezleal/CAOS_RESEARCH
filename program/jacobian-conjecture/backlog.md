...
| JCB-036 | The rotation induction: formalize the normal form for x-swallowed components (dominating direction onto a coordinate by linear gauge), control the excluded shapes under rotation, handle ties; target: every component is triangular after finitely many rotations (= JC(2) route). Machine: scan small x-swallowed components, verify each rectifies by one rotation | P5 | todo | 2026-07-22 | the classification induction |
