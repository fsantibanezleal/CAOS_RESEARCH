...
| JCB-037 | The corner calculus: prove hard-shape (mixed-corner) pairs cannot be Keller. Data so far: 8 swallowed-mixed windows empty; certificates are MULTI-ray (not a single residue). Route: pair-level corner constraints (both components cornered), two-corner cascade, weighted multi-class systems | P5 | todo | 2026-07-22 | the open core, now with instruments |
