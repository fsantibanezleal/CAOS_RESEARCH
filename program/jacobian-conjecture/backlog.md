...
| JCB-038 | Theorem 5: window form (EXP-042) + all-degree [D] via THE CERTIFICATE TOWER (EXP-044: -2a window-independent; tower single-ratio; forward-only injection). REMAINING: the TOWER LEMMA (ratio a nonzero multiple of a for all N) | P5 | doing | 2026-07-22 | one lemma from unconditional |
...
| JCB-040 | Frontier: first contact (EXP-043) + mid-scale certificates and the x18.8 filter (EXP-045): the (48,64) sweep is SMALL (111 unknowns, blocks <= 13). NEXT: run the sampled sweep, then B = 16 beyond max 150, then (72,108) | P5 | doing | 2026-07-22 | scheduling, not risk |
