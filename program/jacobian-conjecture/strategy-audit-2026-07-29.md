...
## Amendment after EXP-112 through EXP-114

The graph-compression gate is decided and the route advances:

- EXP-112 reduces the selected 125-column determinant exactly to one
  36-column block depending on 24 parameters, plus three forced singleton
  factors. The other 27 parameters are acyclic and determinant-inert on this
  chart.
- EXP-113 proves there is no further SCC split after removing the forced
  direction. Full 36-vertex connectivity is already generated by either of
  two explicit three-parameter supports.
- EXP-114 proves graph support can overestimate determinant dependence. On
  \(T_A=\{(0,1),(0,7),(2,9)\}\), the determinant is independent of
  \((2,9)\) and factors into degrees \(3,12,6\).
- On \(T_B=\{(0,1),(0,5),(1,0)\}\), after
  \(d=1+\varepsilon_{(1,0)}\), the determinant factors into weighted
  homogeneous polynomials of weights \((7,3,9)\) and weighted degrees 54 and
  63.

The next P0 step is a weighted residual-chart cover for the two \(T_B\)
factors, split into \(d\ne0\) and \(d=0\), using alternative minors from the
complete 302-row system. Graph-only compression is complete on the selected
pinned basis.
...
|---|---|---|---|
| Complete augmented-matrix graph compression | done at pinned-chart scope | EXP-112 gives an exact 125-to-36 and 51-to-24 reduction | no further SCC split on this basis after EXP-113 |
| Weighted residual charts | P0, pursue now | EXP-114 gives two primitive weighted factors on \(T_B\) | split \(d\ne0\) and \(d=0\), then select alternative complete-row minors |
| Constructible augmented-minor stratification | P0, retain in compressed form | exact framework for exceptional loci, validated on slices and compressed by EXP-112 through EXP-114 | operate on exact factor loci, not the raw 51-variable family |
