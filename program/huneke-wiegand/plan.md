...
of each rational/mod-two rank gap with its first Bockstein rank proves the
complete 2-primary groups are elementary. Therefore the two mask-58 endpoint
triangles vanish integrally; all other triangle images form its complete
2-primary subgroup. All triangles generate the complete 2-primary subgroups
of masks 59/62. Mask 56 is trivial through `p=10` and generated by `(2,3,4)`
at `p=11`.

The strongest next route is constructive and uniform: derive explicit
mask-58 sources for `(0,1,p-3)` and `(0,2,p-4)` through the contraction, and
transport EXP-062's parity functionals to duals for every remaining triangle.
The finite exponent theorem proves what the map does but not a formula for the
map. After that, attack the complementary quotient/upper bound; recurrence
remains third and mask 56 remains separate.

No manuscript or Zenodo update follows from this finite theorem. A uniform
constructive comparison, normal form, or complementary-cokernel theorem is
still the publication threshold. No next experiment is declared here.

## Superseded priority after EXP-063, 2026-09-20

EXP-063 now supplies the previously missing finite labelled target bridge.
For `p=8,9,10,11`, all 19 exact EXP-062 triangle rows survive the certified
unit contraction literally as semantic `R5` coordinates. Masks 59 and 62 have
triangle quotient ranks `3,4,5,7` with no relations. Mask 58 has ranks
`1,2,3,5`, with complete relation space generated by `(0,1,p-3)` and
`(0,2,p-4)` throughout the tested range. The independent audit reconstructs
every component and exhausts every triangle subset.

The new strongest route is not a larger sweep. It is a uniform integral
relative calculation for precisely those two endpoint classes: construct
exact mask-58 witnesses and transformed duals, decide integral zero versus
divisibility by a possible order-four class, and only then assemble a uniform
comparison diagram. After that, seek the complementary quotient/upper bound;
the recurrence remains third. EXP-063's mod-two relation is not an integral
kernel theorem, and mask 56 remains a separate threshold problem.

No manuscript or Zenodo update follows from this finite bridge. A uniform
integral comparison, normal form, or complementary-cokernel theorem remains
the publication threshold. No EXP-064 is declared in this plan update.

## Superseded priority after EXP-062, 2026-09-05
