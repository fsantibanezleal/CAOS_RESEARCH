...
| HWB-035 | 2 | in progress | interior graded Betti table of the conductor fiber cone | EXP-027/028 complete the entire second row; EXP-029 completes internal degree five; EXP-030 determines `beta_(3,6)`; `beta_(3,7)` and the remaining interior table stay open |
...
| HWB-045 | 2 | in progress | final third-row entry and integral contraction | EXP-031 tests the all-parameter prediction `beta_(3,7)=0` by zero-vertex Morse matching, unit tetrahedral fillers, complete small profiles, and an independent audit |
