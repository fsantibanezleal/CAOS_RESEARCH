...
| HWB-065 | 1 | in progress | test whether the first discrepancy is a degree-six relation in the parity-defect module | audited exact `e_11=102` and `e_12=138` pass, but EXP-039/040 show that support components merge and exchange latent sector identity; an explicit relation/recurrence proof remains mandatory |
...
| HWB-068 | 1 | in progress | identify semantic sector tags across support mergers | EXP-041 is declared before implementation: exact interval-atom fingerprints must reproduce the frozen components and decide whether the isolated block switches from the `p-5` sector to a `p-4` sector before bridge deletion |
