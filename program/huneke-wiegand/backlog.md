...
| HWB-068 | 1 | pending | identify semantic sector tags across support mergers | Profile interval-labelled rows/columns in the defective `p=8,...,11` components and decide whether the isolated block switches from the `p-5` sector to a `p-4` sector before attempting bridge deletion |
