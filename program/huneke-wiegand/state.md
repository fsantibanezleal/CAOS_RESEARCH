...
EXP-063 is CONFIRMED FINITELY for `p=8,...,11`. Every one of the 19 exact
triangle rows survives the unit contraction literally as an `R5` coordinate.
Masks 59 and 62 retain mod-two quotient ranks `3,4,5,7` with no triangle
relations. Mask 58 has ranks `1,2,3,5`, and its complete finite relation space
is uniformly generated by `(0,1,p-3)` and `(0,2,p-4)`. Mask 56 has ranks
`0,0,0,1`. Independent exhaustive subset enumeration and 12 controls pass.

EXP-064 upgrades this to exact integral finite structure. Sixteen
modular-Hadamard certificates prove rational ranks, and every rational/mod-two
rank gap equals the first Bockstein rank. Hence all four carrier families have
elementary 2-primary torsion in the tested range. The two mask-58 endpoint
triangles vanish integrally; the remaining triangles generate its complete
2-primary subgroup. Masks 59/62 are completely generated by all triangles;
mask 56 is trivial through p=10 and generated by `(2,3,4)` at p=11. The
independent audit recomputes 1,100 prime/rank pairs and every consequence.
...
Still open: a uniform **constructive integral** comparison to the relative-completion
presentations, primitive-complement/normal-form upper bounds, and lower-strand
recurrence. EXP-063 identifies the finite target coordinates but its mod-two
relations now become integral vanishings through EXP-064's exponent theorem.
Next rank is explicit all-parameter witnesses and transformed duals for the two
