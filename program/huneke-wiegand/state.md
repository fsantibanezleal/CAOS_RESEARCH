...
| 2026-08-12 | published | published | PR #164 passed `guards` and `test` and merged EXP-019 plus v0.09 to `develop` at `efdca94`; PR #165 passed all checks and promoted the identical tested tree to `main` at `cbe62a5` |
...
| 2026-08-12 | published | published | PR #172 passed `guards` and `test` and merged EXP-021 plus v0.11 to `develop` at `178a7361`; PR #173 passed all checks and promoted the identical tested tree to `main` at `7abd1040` |
| 2026-08-17 | published | active | EXP-022 declared before implementation: use the published defining-ideal construction and exact value congruences to test whether `J_p` is generated by `50p^2-17p` quadrics |
| 2026-08-17 | active | active | EXP-022 REFUTED quadratic generation: the universal isolated relation `X_0^2X_(3p)-X_p^3` proves `beta_(1,3)>=1`; complete `p=4,5,6` runs find exactly one cubic and no degree-four/five equations |
...
EXP-049 separates the mod-two and integral claims. Every literal equation `Ry=2a` for the four
...
EXP-053 tests the missing pullback. At `p=8,9,10`, exact label reconstruction is unique and the
same source chain gives both completion boundaries, with supports 125, 178, and 238 and
coefficients bounded by four. Seventy-eight semantic skeletons refute the compact HNF-pullback
hypothesis, and `p=10` completes only after a transformed-HNF resource overrun. HWB-078 seeks a
direct labelled telescoping chain whose boundary is the union of both frozen EXP-052 formulas on
mask `63`.
...
tree `8ea3fbd0dfd136a7b91c508a31146be7d88eded1` through `develop` and `main`. EXP-035 is confirmed
under completed HWB-058. It proves the all-parameter
...
EXP-034 proves the exact class

```text
beta_(p,(p+2,8p-1+p(p+1)/2))(K_p)
=beta_(p,(p+2,8p-1+p(p+1)/2))(A_p)
=beta_(p,(p+2,8p-1+p(p+1)/2))(C_p)=1.
```

The complete two lower strands of `A_p`, and hence the full resolution of `C_p`, remain open. For
...
Its support is `[15p+1,39p-3] minus {33p-1}`. With EXP-028 and the exact Hilbert numerator, this
completes internal degree five over every field:
...
The 297-row campaign passes through `p=300`; complete exact `H_2` profiles at `p=4,5,6` total
