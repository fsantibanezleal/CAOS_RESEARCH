...
> generic names. In the tree actually shipped: **`productlab/` = `data-pipeline/researchlab/`** (the Python
> package lives INSIDE `data-pipeline/`; on instantiation rename `researchlab` → `<slug>lab`), **`web/` =
