...
\date{2026-07-22 (working manuscript, v0.01; split from the foundational companion)}
...
executable elimination whose obstruction always localizes at the constant's class, and prove a
window form of the corresponding fifth exclusion with cleared all-parameter certificates
(monomial pairings such as $-13860\,a^7$). A source-audited recalibration places the program
...
\begin{conjecture}[{[C]}; the fifth exclusion, all degrees]\label{conj:t5}
The constant-class transport equation is a nonzero monomial multiple of $a$ at every
window: no above-weight perturbation of $x + a x^u y^v$ is ever a Keller component. The
missing step is the closed form of the transport chain, the staircase analog of the banded
formula behind Theorem~\ref{thm:t1}.
\end{conjecture}
...
land in Theorem~\ref{thm:t5}'s territory (sampled windows empty). (ii) For the $(72, 108)$
system, the classwise decomposition is block-cheap: exact histograms give $5992$ window
unknowns in $535$ classes with largest diagonal block $22$ (and $2142$ unknowns, largest
block $13$, for the $(48, 64)$ validation case): the attack cost is coupling bookkeeping,
not linear algebra, and the transport instrument automates exactly that.
...
Priorities, from the standing routes evaluation: (1) close
Conjecture~\ref{conj:t5} by the transport chain's closed form (the cleared-certificate and
annihilation ingredients are already in place); (2) run the transport sweep on the
$(48, 64)$ validation case and then the open $B = 16$ territory and $(72, 108)$; (3) the
properness reformulation ($JC(2)$ holds iff every planar Keller map has empty Jelonek set;
exact certificates shipped, EXP-014) as a cross-check instrument. The reproducibility
