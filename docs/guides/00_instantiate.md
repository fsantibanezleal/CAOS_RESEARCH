...
2. **Rename** the package `researchlab` -> `<slug>lab` (the folder + all imports + `pyproject.toml`
   `[tool.setuptools.packages.find].where`/name + the scripts' `-m researchlab.pipeline` + docs).
