...
import { decimalCenter as center, localSelbergEvidence, parityEvidence, pressureWinner } from '../lib/riemannReplay';
...
export default function RiemannHypothesis() {
  ...
  useEffect(() => {
    ...
    loadExperiments().then((value) => { if (active) setExps(value.filter((e) => e.problem === 'riemann-hypothesis' && ['001', '002', '003', '004', '005'].includes(e.id))); }).catch(() => { if (active) setRecordsError(true); });
    return () => { active = false; };
  }, [attempt]);
  ...
  const tabs: TabDef[] = [
    {
      id: 'summary', label: t('Summary', 'Resumen'), content: <section>
        <p className="rh-lead">{t('EXP-005 gives an explicit improvement in the interval exponent. Localizing Pearce-Crump’s optimized Selberg detector proves a positive density of distinct odd-multiplicity critical zeros for every fixed θ > 1/2. The parity transfer then puts the simple-critical positivity threshold in the certified interval (0.5459, 0.546), below Wang’s cosine threshold 0.550193964744154… .', 'EXP-005 da una mejora explícita del exponente de los intervalos. Localizar el detector optimizado de Selberg de Pearce-Crump prueba una densidad positiva de ceros críticos distintos de multiplicidad impar para cada θ fijo mayor que 1/2. La transferencia de paridad sitúa entonces el umbral de positividad de ceros críticos simples en el intervalo certificado (0.5459, 0.546), bajo el umbral coseno 0.550193964744154… de Wang.')}</p>
        <Equation tex={String.raw`\liminf_{T\to\infty}\frac{O(T,T^\theta)}{N(T,T^\theta)}\ge\frac{\theta-1/2}{4eC_3},\qquad \liminf_{T\to\infty}\frac{S(T,T^\theta)}{N(T,T^\theta)}\ge\max\left\{0,c(\theta),\frac{c(\theta)+(\theta-1/2)/(2eC_3)}3\right\}`} />
        {local ? <>
        ...
        <Callout variant="honest" title={t('What the result establishes', 'Qué establece el resultado')}>
          {t('The confirmed repository deduction includes an explicit short-interval positivity threshold in (0.5459, 0.546), together with the earlier pressure refinement at θ = 3/4. RH remains open. No effective starting height, global record or universal simplicity follows. The manuscript awaits external review; automated audits, exact certificates and publication do not constitute peer acceptance or an end-to-end formal proof.', 'La deducción confirmada en el repositorio incluye un umbral explícito de positividad en intervalos cortos dentro de (0.5459, 0.546), junto con el refinamiento anterior por presión para θ = 3/4. RH sigue abierta. No se obtiene una altura inicial efectiva, un récord global ni simplicidad universal. El manuscrito espera revisión externa; las auditorías automatizadas, los certificados exactos y la publicación no constituyen aceptación por pares ni una prueba formal de extremo a extremo.')}
        </Callout>
        <div className="rh-source-links"><SourceLink href={LOCAL_PROOF}>{t('Full EXP-005 localization proof', 'Prueba completa de localización de EXP-005')}</SourceLink><SourceLink href={PARITY_PROOF}>{t('Full EXP-004 parity proof', 'Prueba completa de paridad de EXP-004')}</SourceLink><SourceLink href={PRESSURE_PROOF}>{t('Full EXP-003 pressure proof', 'Prueba completa de presión de EXP-003')}</SourceLink><SourceLink href={PAPER}>{t('Read the manuscript PDF', 'Leer el manuscrito PDF')}</SourceLink><SourceLink href={DOI}>{t('Current Zenodo version (0.05)', 'Versión actual en Zenodo (0.05)')}</SourceLink><SourceLink href={CONCEPT_DOI}>{t('Zenodo record and versions', 'Registro y versiones de Zenodo')}</SourceLink><SourceLink href="https://doi.org/10.5281/zenodo.22727389">{t('First published version (0.01)', 'Primera versión publicada (0.01)')}</SourceLink></div>
    ...
    },
    {
      id: 'results', label: t('Experiments & results', 'Experimentos y resultados'), content: <section>
        ...
        <p>{t('EXP-001 audits source constants and normalization. EXP-002 proves the first short-interval refinement. EXP-003 adds odd-frame pressure. EXP-004 supplies the parity transfer. EXP-005 localizes the optimized Selberg detector and certifies the explicit threshold below 0.546. Open a record to read its declaration, verdict, artifacts and source history.', 'EXP-001 audita constantes y normalización. EXP-002 prueba el primer refinamiento en intervalos cortos. EXP-003 agrega presión con marcos impares. EXP-004 aporta la transferencia de paridad. EXP-005 localiza el detector optimizado de Selberg y certifica el umbral explícito bajo 0.546. Abra un registro para leer su declaración, veredicto, artefactos e historia.')}</p>
        <ul className="rh-experiments">{exps.map((e) => <li key={e.slug}><button className="rs-exp-open" onClick={() => setOpen(e)}>EXP-{e.id}: {e.id === '001' ? t('Source and constant audit', 'Auditoría de fuentes y constantes') : e.id === '002' ? t('Short-interval stability refinement', 'Refinamiento por estabilidad en intervalos cortos') : e.id === '003' ? t('Odd-frame pressure refinement', 'Refinamiento por presión con marcos impares') : e.id === '004' ? t('Parity density transfer', 'Transferencia de densidad por paridad') : t('Local Selberg transfer', 'Transferencia local de Selberg')}</button><span className="rs-badge state">{e.verdict || t('Record available', 'Registro disponible')}</span></li>)}</ul>
        ...
        {resultTable}
        <p>{t('The baseline is Wang’s short-interval theorem. EXP-003 derives numerical refinements at θ = 3/4. EXP-004 gives the parity transfer. EXP-005 replaces the unspecified local odd-zero density by an explicit curve and certifies the new threshold bracket.', 'La cota base es el teorema de Wang en intervalos cortos. EXP-003 deriva refinamientos numéricos para θ = 3/4. EXP-004 da la transferencia de paridad. EXP-005 reemplaza la densidad local no especificada de ceros impares por una curva explícita y certifica el nuevo intervalo del umbral.')} <Cite id="riemann-wang2026" /> <Cite id="riemann-pressure2026" /> <Cite id="riemann-local2026" /></p>
    ...
    },
    {
      id: 'open', label: t('Open questions', 'Preguntas abiertas'), content: <section>
        ...
        <ol className="rh-questions">
          ...
          <li><h3>{t('Push the explicit threshold toward one half', 'Acercar el umbral explícito a un medio')}</h3><p>{t('EXP-005 lowers the explicit threshold to the interval (0.5459, 0.546), but the certified proportion is small there. A stronger diagonal profile, a reproducible higher-rank Selberg optimizer, or a sharper parity inequality could move the unique zero closer to 1/2.', 'EXP-005 reduce el umbral explícito al intervalo (0.5459, 0.546), pero la proporción certificada allí es pequeña. Un perfil diagonal más fuerte, un optimizador de Selberg de mayor rango reproducible o una desigualdad de paridad más precisa podrían acercar la raíz única a 1/2.')}</p></li>
