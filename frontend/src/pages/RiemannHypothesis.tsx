...
import { decimalCenter as center, hilbertParityEvidence, localSelbergEvidence, parityEvidence, pressureWinner } from '../lib/riemannReplay';
...
export default function RiemannHypothesis() {
  ...
  useEffect(() => {
    ...
    loadExperiments().then((value) => { if (active) setExps(value.filter((e) => e.problem === 'riemann-hypothesis' && ['001', '002', '003', '004', '005', '006'].includes(e.id))); }).catch(() => { if (active) setRecordsError(true); });
    return () => { active = false; };
  }, [attempt]);
  const result = data?.result;
  // Keep a readable fallback while an older baked replay is being replaced by
  // the v4 export. The page must not throw when the optional new branch is
  ...
  const tabs: TabDef[] = [
    {
      id: 'summary', label: t('Summary', 'Resumen'), content: <section>
        <p className="rh-lead">{t('EXP-006 keeps the first Hilbert-subspace dimension before scalar compression and couples it to odd multiplicity. Combined with the localized Selberg curve from EXP-005, it places the simple-critical positivity threshold in the certified interval (0.545884, 0.545885). At θ = 0.5459 it proves a positive proportion where the earlier linear transfer is still negative.', 'EXP-006 conserva la dimensión del primer subespacio de Hilbert antes de la compresión escalar y la vincula con la multiplicidad impar. Combinada con la curva local de Selberg de EXP-005, sitúa el umbral de positividad de ceros críticos simples en el intervalo certificado (0.545884, 0.545885). En θ = 0.5459 prueba una proporción positiva donde la transferencia lineal anterior sigue siendo negativa.')}</p>
        <Equation tex={String.raw`(Q-S)(N-O)\ge2(N-S)^2,\qquad h_3(\theta)=\frac{3+k_3(\theta)-\sqrt{(1-k_3(\theta))(9-k_3(\theta)-8c(\theta))}}4`} />
        {hilbert ? <>
          <p className="rh-parameters">θ = <b>{hilbert.result.parameters.theta.decimal}</b><br />θ<sub>HP</sub> ∈ (<b>{hilbert.result.parameters.root_lower_theta.decimal}</b>, <b>{hilbert.result.parameters.root_upper_theta.decimal}</b>)</p>
          <p className="rh-number">{t('Certified simple-critical lower bound at θ = 0.5459: ', 'Cota inferior certificada de ceros críticos simples en θ = 0.5459: ')}<strong>{hilbert.result.target.strong_simple_lower.decimal}</strong></p>
          <p className="small">{t('Scientific verdict: confirmed after universal-proof, analytic-transfer, scalar-barrier, and exact-certificate review.', 'Veredicto científico: confirmado tras revisar la prueba universal, la transferencia analítica, la barrera escalar y el certificado exacto.')}</p>
        </> : <p role="status">{t('Loading the source-bound EXP-006 evidence.', 'Cargando la evidencia de EXP-006 vinculada a sus fuentes.')}</p>}
        <div className="rh-source-links"><SourceLink href={HILBERT_PROOF}>{t('Full EXP-006 proof', 'Prueba completa de EXP-006')}</SourceLink><SourceLink href={`${REPO}/blob/main/${HILBERT_EXP}/verdict.md`}>{t('Confirmed EXP-006 verdict', 'Veredicto confirmado de EXP-006')}</SourceLink></div>
        <h2>{t('Localized odd-support input', 'Entrada local de soporte impar')}</h2>
        <Equation tex={String.raw`\liminf_{T\to\infty}\frac{O(T,T^\theta)}{N(T,T^\theta)}\ge k_3(\theta)=\frac{\theta-1/2}{4eC_3}`} />
        {local ? <>
    ...
    },
    {
      id: 'results', label: t('Experiments & results', 'Experimentos y resultados'), content: <section>
        ...
        <p>{t('EXP-001 audits source constants and normalization. EXP-002 proves the first short-interval refinement. EXP-003 adds odd-frame pressure. EXP-004 supplies the parity transfer. EXP-005 localizes the optimized Selberg detector. EXP-006 retains the Hilbert dimension and certifies the threshold below 0.545885. Open a record to read its declaration, verdict, artifacts and source history.', 'EXP-001 audita constantes y normalización. EXP-002 prueba el primer refinamiento en intervalos cortos. EXP-003 agrega presión con marcos impares. EXP-004 aporta la transferencia de paridad. EXP-005 localiza el detector optimizado de Selberg. EXP-006 conserva la dimensión de Hilbert y certifica el umbral bajo 0.545885. Abra un registro para leer su declaración, veredicto, artefactos e historia.')}</p>
        <ul className="rh-experiments">{exps.map((e) => <li key={e.slug}><button className="rs-exp-open" onClick={() => setOpen(e)}>EXP-{e.id}: {e.id === '001' ? t('Source and constant audit', 'Auditoría de fuentes y constantes') : e.id === '002' ? t('Short-interval stability refinement', 'Refinamiento por estabilidad en intervalos cortos') : e.id === '003' ? t('Odd-frame pressure refinement', 'Refinamiento por presión con marcos impares') : e.id === '004' ? t('Parity density transfer', 'Transferencia de densidad por paridad') : e.id === '005' ? t('Local Selberg transfer', 'Transferencia local de Selberg') : t('Hilbert-parity compression', 'Compresión de Hilbert y paridad')}</button><span className="rs-badge state">{e.verdict || t('Record available', 'Registro disponible')}</span></li>)}</ul>
        ...
        {resultTable}
        <p>{t('The baseline is Wang’s short-interval theorem. EXP-003 derives numerical refinements at θ = 3/4. EXP-004 gives the parity transfer. EXP-005 supplies the explicit local odd-zero curve. EXP-006 strengthens the finite transfer and certifies the new threshold bracket.', 'La cota base es el teorema de Wang en intervalos cortos. EXP-003 deriva refinamientos numéricos para θ = 3/4. EXP-004 da la transferencia de paridad. EXP-005 aporta la curva local explícita de ceros impares. EXP-006 fortalece la transferencia finita y certifica el nuevo intervalo del umbral.')} <Cite id="riemann-wang2026" /> <Cite id="riemann-pressure2026" /> <Cite id="riemann-local2026" /></p>
    ...
    },
    {
      id: 'open', label: t('Open questions', 'Preguntas abiertas'), content: <section>
        ...
        <ol className="rh-questions">
          ...
          <li><h3>{t('Push the explicit threshold toward one half', 'Acercar el umbral explícito a un medio')}</h3><p>{t('EXP-006 lowers the explicit threshold to (0.545884, 0.545885), but the certified proportion is small there. A stronger reproducible odd-support profile or a new analytic input coupled to multiplicity geometry could move the unique zero closer to 1/2. The printed rank-six constant remains sensitivity only until its full profile is available.', 'EXP-006 reduce el umbral explícito a (0.545884, 0.545885), pero la proporción certificada allí es pequeña. Un perfil reproducible más fuerte de soporte impar o una nueva entrada analítica vinculada con la geometría de multiplicidades podría acercar la raíz única a 1/2. La constante impresa de rango seis sigue siendo solo sensibilidad hasta que esté disponible su perfil completo.')}</p></li>
