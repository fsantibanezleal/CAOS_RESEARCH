      ...
      content: (
        <section>
          ...
          <Callout variant="note" title={t('What the search found on the known graphs (EXP-010)', 'Lo que la busqueda hallo en los grafos conocidos (EXP-010)')}>
            {t(
              'Every 4-pole obtained from the two 52-vertex graphs and from the 68-vertex graph by deleting two independent edges is Petersen colorable (482, 482 and 4,947 orbit representatives), with exactly the two boundary patterns the cut space allows: two crossed equal pairs, or the four edges around one edge of the Petersen graph. Adjacent labels are always available at the ends of a deleted edge, so chains of such 4-poles are colorable. Dot products of the 52-vertex graph with itself are new 102-vertex counterexamples, all with defect 2. The question stays open.',
              'Todo 4-polo obtenido de los dos grafos de 52 vertices y del de 68 vertices al borrar dos aristas independientes es coloreable por Petersen (482, 482 y 4.947 representantes de orbita), con exactamente los dos patrones de borde que el espacio de cortes permite: dos pares iguales cruzados, o las cuatro aristas alrededor de una arista del grafo de Petersen. En los extremos de una arista borrada siempre hay etiquetas adyacentes disponibles, asi que las cadenas de tales 4-polos son coloreables. Los productos punto del grafo de 52 vertices consigo mismo son contraejemplos nuevos de 102 vertices, todos con defecto 2. La pregunta sigue abierta.',
            )}
          </Callout>
