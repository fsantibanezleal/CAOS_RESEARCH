  ...
  const tabs: TabDef[] = [
    {
      ...
      content: (
        <section>
          ...
          <p>
            {t(
              'This program validated the counterexample independently in exact arithmetic and then built on it: the structure, an infinite family, exact escape geometry, characteristic-p certificates, a dimension-2 rigidity theorem, an explicit dimension-48 Hessian witness, and an unconditional theorem ladder for the planar case. It runs a machine campaign on the last open degree pair below 125, (72,108) (no closing certificate exists at degrees 1 or 2), and re-scoped the [125,150] frontier. A structural result: the 2026 counterexample is the incidence map choosing a root of a form, and its exact mechanism is provably excluded in the plane (its source is not affine 2-space), controlled by a single invariant. Ninety experiments, each with a persisted verdict, refutations and one retraction kept in the record; the two-variable case remains open and is the standing target.',
              'Este programa valido el contraejemplo de forma independiente en aritmetica exacta y construyo sobre el: la estructura, una familia infinita, la geometria de escape exacta, certificados en caracteristica p, un teorema de rigidez en dimension 2, un testigo Hessiano explicito en dimension 48 y una escalera de teoremas incondicionales para el caso plano. Corre una campana de maquina sobre el ultimo par de grados abierto bajo 125, (72,108) (no existe certificado de cierre en grados 1 o 2), y re-delimito la frontera [125,150]. Un resultado estructural: el contraejemplo de 2026 es el mapa de incidencia que elige una raiz de una forma, y su mecanismo exacto queda excluido en el plano (su fuente no es el 2-espacio afin), gobernado por un solo invariante. Noventa experimentos, cada uno con veredicto persistido, con refutaciones y una retractacion en el registro; el caso de dos variables sigue abierto y es el objetivo permanente.',
            )}
          </p>
    ...
    },
    {
      ...
      content: (
        <section>
          ...
          <p>
            {t(
              'What stays open after the cascade: the two-variable Jacobian conjecture; Dixmier and Poisson at ranks 1 and 2; Mathieu for SU(3) to SU(23) and for the other compact groups (it fails for SU(2) and for every SU(N) with N >= 24); the minimal failing dimensions of the vanishing and Image statements (the explicit witness of EXP-041 bounds the first at 48 variables); and GMC(2), for which a proof is claimed.',
              'Lo que sigue abierto tras la cascada: la conjetura jacobiana en dos variables; Dixmier y Poisson en rangos 1 y 2; Mathieu para SU(3) a SU(23) y para los demas grupos compactos (falla para SU(2) y para todo SU(N) con N >= 24); las dimensiones minimas de falla de los enunciados de anulacion e imagen (el testigo explicito de EXP-041 acota la primera en 48 variables); y GMC(2), para la que se anuncia una demostracion.',
            )}
          </p>
